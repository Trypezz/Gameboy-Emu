/*
 * How to read the optcode-table:
 *
 *        INS   REG
 *         2     4
 *         Z N H C
 *            ^
 * (All of them are set by default because anything 'could' happen)
 *
 *
 * (Name of the instruction) (Affected Registers)
 *    (Length in Bytes)   (Number of machine cycles)
 *                (Flags affected)
 *
 *  Flags:
 *  Z = Zero Flag -> Sets when the result of an operation is 0
 *  N = Substract Flag -> Sets when the instruction was a substraction
 *  H = Half Carry Flag ->
 *  C = Carry Flag ->
 *  0 = The flag is reset
 *  1 = The Flag is set
 ...
 *  Example:
 *
 *   ADD A,D  (ADD Operation, Value of D onto A)
 *    1   4   (1 Byte in Length | 4 Machine cycles)
 *   Z 0 H C  (Zero could happen, Substracttion is set to 0 because we ADD,
 *            Half Carry could happen, Carry could happen)
 *
 *  Bit meanings:
 *  n8  = immediate 8-Bit data
 *  n16 = immediate little-endian 16-bit data
 *  a8  = 8-bit unsigned data, which is added to $FF00 in certain
...
const struct gb_cpu_instruction instructions[256] = {
    ...
    {"LD L, n8", 1, cpu_ld_l_n8},        /* 0x2E */
    {"CPL", 0, nullptr},                 /* 0x2F */
    {"JR NC, e8", 1, nullptr},           /* 0x30 */
    {"LD SP, n16", 2, cpu_ld_sp_n16},    /* 0x31 */
    ...
    {"ADD A, B", 0, cpu_add_b_to_a},     /* 0x80 */
    {"ADD A, C", 0, cpu_add_c_to_a},     /* 0x81 */
    {"ADD A, D", 0, cpu_add_d_to_a},     /* 0x82 */
    {"ADD A, E", 0, cpu_add_e_to_a},     /* 0x83 */
    {"ADD A, H", 0, cpu_add_h_to_a},     /* 0x84 */
    {"ADD A, L", 0, cpu_add_l_to_a},     /* 0x85 */
